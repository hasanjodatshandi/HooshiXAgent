...
### Agent state transaction recovery

RA-5 adds a blocking Agent-local state acceptance to the final release chain. It verifies that `init`/`configure` do not leave mismatched config and credential generations after injected failures, interrupted journals fail read paths closed and are recovered by the next serialized mutation, `status`/`doctor` do not create or rewrite state, stale locks are reclaimed only with safe ownership evidence, permanent local/auth-policy failures stop reconnecting, and runtime error rendering does not disclose session-token/credential material. The platform secret format, Agent-to-Gateway protocol and external Control Panel authority remain unchanged.

### Filesystem and installer trust

RA-6 adds blocking filesystem trust acceptance for static/live Gateway metadata and Agent state/secrets. Metadata/state files are opened with platform no-follow semantics and must be regular files; metadata/state directory components reject Unix symlinks and Windows reparse points; Unix private-mode failures fail closed. The Windows matrix additionally injects rollback failure before replacement and requires both the current and previous binaries to remain unchanged, then verifies a successful transactional rollback.

### Multi-host public edge

RA-4 adds a blocking real Compose/Caddy acceptance for ADR-0011. The production/default Caddyfile must use restricted On-Demand TLS with an external `ask` permission URL and must fail configuration when that URL is absent rather than falling back to unrestricted issuance. The gate uses a test-only internal issuer and permission mock to avoid public CA traffic while preserving the production authorization structure. It proves two approved hostnames simultaneously reach two distinct Agent-owned local endpoints, a TLS-approved hostname with no Gateway route remains `404`, an unknown hostname cannot complete TLS or appear in certificate storage, public `/readyz` and `/metrics` remain `404` while a tenant `/healthz` still reaches the routed tenant application, and both dynamic and static compatibility configurations preserve certificate-verified Caddy→Gateway HTTPS. No Control Panel permission service or domain-ownership business logic is implemented in this repository.
...
- Linux `systemd --user`;
- macOS LaunchAgent;
- Windows: the `HooshiXAgent` service under the LocalSystem default account (ADR-0014).

The Windows verification above is honest about its limits. Platform CI asserts the accepted persistence definition on Windows: the persistence spec asserted there is the `sc.exe create HooshiXAgent ... start= auto` / `service run-service` service model, and `schtasks`/`ONLOGON` are explicitly rejected. A gate that installs, asserts, restarts and uninstalls the real service distribution also exists — `scripts/ci/windows-service-install-smoke.ps1` (ADR-0014 rollout step R4), with an elevated real-machine run recorded on 2026-09-17 — and it is now **invoked by the `windows-distribution-build` CI job**, which passes the distribution it already built via `-SetupExe`. The gate fails closed rather than skipping, so a hosted Windows run that could not install, start, restart or uninstall the service fails that job by name. Because no workflow run has been observed since the wiring landed, automated CI evidence for Windows service installation and reboot persistence is pending the next Windows CI run and must not be reported as `Passed` before then; the build-integrity steps of that job are unchanged. `docs/engineering/executable-runtime-gate.md` section 7 (G1) records the gate and its one residual limit: it performs no literal physical OS reboot.
