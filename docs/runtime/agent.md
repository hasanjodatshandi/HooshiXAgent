...
Agent configuration loading is strict: `config.json` must contain exactly one known-field JSON object and only trailing whitespace after it. A second JSON value, unknown field, symlinked config file, unsupported version, or malformed JSON fails closed. Configuration writers use an atomic temporary-file replacement and a bounded cross-process `.config.lock` directory so `configure` and `expose add/remove` perform their read-modify-write operation under one lock instead of silently overwriting concurrent changes. If a process crashes while holding the lock, later mutations fail closed after the bounded wait; an operator may remove a stale `.config.lock` only after confirming no Agent CLI mutation is running.
