...
func (exporter *statusExporter) emit(signal contractv1.GatewayStatusSignal) {
	...
	case <-ctx.Done():
		exporter.failures.Add(1)
		// A sink may ignore context cancellation. Latch the exporter off after
		// its first timeout so at most one non-cooperative Emit goroutine can
		// remain for this exporter.
